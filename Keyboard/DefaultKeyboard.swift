...
//
//  Created by Shashank Patel on 3/22/16.
//  Copyright (c) 2014 Shashank Patel. All rights reserved.
//

func defaultKeyboard() -> Keyboard {
    let defaultKeyboard = Keyboard()
    for key in ["Calle", "Avenida", "Callejón", "Boulevard", "Senda", "Pasaje", "Peatón"] {
    ...
    defaultKeyboard.addKey(backspace, row: 4, page: 0)
    
    let keyModeChangeNumbers = Key(.ModeChange)
    keyModeChangeNumbers.uppercaseKeyCap = "123"
    keyModeChangeNumbers.toMode = 1
    defaultKeyboard.addKey(keyModeChangeNumbers, row: 5, page: 0)
    
    let keyboardChange = Key(.KeyboardChange)
    defaultKeyboard.addKey(keyboardChange, row: 5, page: 0)
    
    let settings = Key(.Settings)
    defaultKeyboard.addKey(settings, row: 5, page: 0)
    
    let space = Key(.Space)
    space.uppercaseKeyCap = "space"
    space.uppercaseOutput = " "
    space.lowercaseOutput = " "
    defaultKeyboard.addKey(space, row: 5, page: 0)
    
    let returnKey = Key(.Return)
    returnKey.uppercaseKeyCap = "return"
    returnKey.uppercaseOutput = "\n"
    returnKey.lowercaseOutput = "\n"
    defaultKeyboard.addKey(returnKey, row: 5, page: 0)
